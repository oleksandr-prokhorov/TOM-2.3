		...
		if cur_lang == "WARAN" then
			...
			for i = 0, GetMaxPlayers() do
				if GetDistancePlayers(id, i) < 600 then
					if GetPlayerWorld(i) == world and GetPlayerVirtualWorld(i) == GetPlayerVirtualWorld(id) then
				...
				elseif GetDistancePlayers(id, i) > 800 and GetDistancePlayers(id, i) <= 1000 then
					if GetPlayerWorld(i) == world and GetPlayerVirtualWorld(i) == GetPlayerVirtualWorld(id) then
						if Player[i]._language_demon ~= 0 then
							if Player[id].zvanie ~= nil then
								if Player[id].masked == true then
									SendPlayerMessage(i, 110, 110, 110, pName..": "..text.." (Язык демонов)")
								else
									SendPlayerMessage(i, 110, 110, 110, Player[id].zvanie.." "..pName..": "..text.." (Язык демонов)");
								end
							else
								SendPlayerMessage(i, 110, 110, 110, pName..": "..text.." (Язык демонов)");
							end
...
function _aGiveLang(id, sets) -- команда выдачи языка
	...
	if Player[id].astatus > 2 then
		...
		if result == 1 then
			if Player[pid].loggedIn == true and IsPlayerConnected(pid) == 1 then
				...
				elseif lang == "ящер" or lang == "waran" then
					if par > 0 then
						...
						_saveLang(pid);
						SendPlayerMessage(pid, 255, 255, 255, "Теперь вы знаете язык Язык ящеров.")
						SendPlayerMessage(id, 255, 255, 255, "Вы выдали язык Язык ящеров игроку "..GetPlayerName(pid)..".")
					else
						Player[pid]._language_waran = par;
						Player[pid]._language_current = "MYRTANA";
						_saveLang(pid);
						SendPlayerMessage(pid, 255, 255, 255, "Теперь вы не знаете язык Язык ящеров.")
						SendPlayerMessage(id, 255, 255, 255, "Вы забрали язык Язык ящеров у игрока "..GetPlayerName(pid)..".")
					end
			...
			SendPlayerMessage(id, 255, 255, 255, "Доступные языки: орк, вар, мир, норд, яркен, демон, тиморис, гатия, юо, ящер");
		end
	end
end
		...
		if Player[id].loggedIn == true then
			...
			elseif lang == "яркен" or lang == "yarken" then
				if Player[id]._language_yarkendar ~= 0 then
					...
					_saveLang(id);
					SendPlayerMessage(id, 255, 255, 255, "Вы установили язык Зодчих (яркендар).")
				else
					SendPlayerMessage(id, 255, 255, 255, "Вы не знаете этот язык.")
				end
			...
			elseif lang == "норд" or lang == "норд" then
				...
				SendPlayerMessage(id, 255, 255, 255, "Доступные языки: орк, вар, мир, норд, демон, тиморис, гатия, юо, ящер, яркен");
			end
		end
	else
