...
function gui_Init(value)
	if type(value) == "boolean" then
		...
		for i = 0, MAX_PLAYERS - 1 do
			Player[i] = {};
			Player[i].Option_ID = {};
			Player[i].Option_Scroll = {};
			Player[i].Open_Box = {};
			Player[i].Open_Input = {};
			Player[i].Open_Menu = {};
			Player[i].Open_Menu["scroll"] = nil;
			Player[i].InputValue = {};
...
function getAmountScroll(playerid, menu_id)
	if GUI then
		for i, k in ipairs(MenuInfo) do
			if k.id == menu_id then
				return Player[playerid].Option_Scroll[menu_id]
			end
		end
	end
		return false;
				...
				for i, k in ipairs(MenuInfo) do
					if k.id == menu_id then
						...
						for j = 1, menu_amount do
							...
							local draw_id;
							if j == 1 then
							OnPlayerCraftDadaya(playerid, k.menu_text[j][1])
						...
						--print(scr_am)
						Player[playerid].Option_Scroll[menu_id] = scr_am
...
function hideGUIMenu(playerid, menu_id)
	if IsNPC(playerid) == 0 and GUI then
		if playerid and menu_id then
			...
			Player[playerid].Option_Scroll[menu_id] = nil
			Player[playerid].OpenedMenu = false;
				...
				if k.id == menu_id then
					...
					for j, v in ipairs(player_draw) do
						if v.id == menu_id and v.type == "menu" then
							...
							OnPlayerCraftDadaya(playerid, k.menu_text[menu_count][1])
...
function OnPlayerCraftDadaya(playerid, text)
end
