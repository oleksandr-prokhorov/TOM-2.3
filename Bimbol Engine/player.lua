...
function GiveItem(playerid, instance, amount)
	...
	if checkPlayerItem(playerid, instance) == false then -- если предмета в инвентаре нет
	
		if #Player[playerid].Items < 15 then -- если есть место в инвентаре
			_GiveItem(playerid, instance, amount);
			giveItem(playerid, instance, amount);
		else -- если нет спавним в мире
			local x, y, z = GetPlayerPos(playerid);
			CreateItem(instance, amount, x, y, z, GetPlayerWorld(playerid), 0, 0, 0, 1);
		end
		
	else -- если предмет уже есть в инвентаре
	
		local slot, invAmount = checkPlayerItem(playerid, instance); -- получаем количество предмета и его слот
		if invAmount < 100 then -- если количество меньше 100

			local cantake = (100 - invAmount); -- считаем сколько уже лежит в инвентаре и сколько можно положить
			if amount <= cantake then -- считаем разницу. если свободное место больше выдаваемого то выдаем
				...
				local rfull = amount - cantake;  -- считаем переизбыток
				_GiveItem(playerid, instance, cantake);
				giveItem(playerid, instance, cantake);
				local x, y, z = GetPlayerPos(playerid);
				CreateItem(instance, rfull, x, y, z, GetPlayerWorld(playerid), 0, 0, 0, 1);
