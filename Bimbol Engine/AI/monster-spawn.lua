	...
	if special_instance and x and y and z and amount and world and distance and type(aggressive) == "boolean" then
		...
		if k then
			...
			for i = 1, amount do
				...
				--SetPlayerColor(bot_id, 3, 142, 255)
				...
				local x_r, z_r = changeValueRandom(math.random(distance)), changeValueRandom(math.random(distance));
				SetPlayerPos(bot_id, x + x_r, y, z + z_r);
				SetPlayerAngle(bot_id, math.random(360));
