...
function createNPC(npc_name, npc_body, npc_bodytex, npc_head, npc_headtex, anim, instance, x, y, z, angle, world, meleeweapon, rangedweapon, armor, vw, item, walk, callback)
	if npc_name and npc_body and npc_bodytex and npc_head and npc_headtex and anim and x and y and z and angle and world then
		...
		SetPlayerName(bot_id, npc_name)
		SetPlayerAdditionalVisual(bot_id, npc_body, npc_bodytex, npc_head, npc_headtex);
		SetPlayerVirtualWorld(bot_id, vw);
		if walk then
			SetPlayerWalk(bot_id, walk)
		end
		if item then
			SetPlayerHand(bot_id, RIGHT_HAND, item)
		end
		if anim then
