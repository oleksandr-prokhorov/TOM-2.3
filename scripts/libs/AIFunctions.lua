...
function AI_SetAivar(_playerid)
    ...
    if(AI_NPCList[_playerid].Aivars["MaxWarnDistance"] == nil)then
        AI_NPCList[_playerid].Aivars["MaxWarnDistance"] = 700;
    end

    if(AI_NPCList[_playerid].Aivars["WarnDistance"] == nil)then
        AI_NPCList[_playerid].Aivars["WarnDistance"] = 600;
    end
    ...
    if(AI_NPCList[_playerid].Aivars["TARGETDISTANCE"] == nil)then
        AI_NPCList[_playerid].Aivars["TARGETDISTANCE"] = 700;
    end
...
function CreateAI_WayNets()
    ...
    AI_WayNets["KOLONIE.ZEN"] = WayNet.create();
    AI_WayNets["KOLONIE.ZEN"]:load("scripts/wps/colony.wp");

    AI_WayNets["FULLWORLD2.ZEN"] = WayNet.create();
    AI_WayNets["FULLWORLD2.ZEN"]:load("scripts/wps/main.wp");
    AI_FreePoints["FULLWORLD2.ZEN"] = FreePointList.create();
    AI_FreePoints["FULLWORLD2.ZEN"]:load("scripts/wps/main.fp");
	
	AI_WayNets["ADDONWORLD2.ZEN"] = WayNet.create();
    AI_WayNets["ADDONWORLD2.ZEN"]:load("scripts/wps/addonworld2.wp");
    AI_FreePoints["ADDONWORLD2.ZEN"] = FreePointList.create();
    AI_FreePoints["ADDONWORLD2.ZEN"]:load("scripts/wps/addonworld2.fp");
    ...
    AI_FreePoints["KOLONIE.ZEN"] = FreePointList.create();
    AI_FreePoints["KOLONIE.ZEN"]:load("scripts/wps/colony.fp");
end
...
function _SpawnNPCpos(_npc, x, y, z, _world)

    if (_npc.id ~= nil and _npc.id == -1) then
    ...
    SetPlayerVirtualWorld(_npc.id, 0);
    SetPlayerWorld(_npc.id, world, "START");
    SetPlayerPos(_npc.id, x, y, z);
...
function SpawnNPC(_npc, _waypoint, _world)
	...
	SetPlayerAngle(_npc.id, math.random(1, 360));
...
function SetEnemy(_playerid, _enemyid)
    if Player[_enemyid].inviseforbots == false then
    	if IsNPC(_enemyid) == 0 and not (AI_PlayerList[_enemyid]) then
            return;
        end
